...
function! statusline#checking() abort
  return get(g:, 'loaded_ale', 0) && ale#engine#IsCheckingBuffer(bufnr('')) ? s:indicator_checking : ''
endfunction

function! statusline#git() abort
    return get(g:, 'loaded_fugitive', 0) ? FugitiveStatusline() : ''
endfunction

function! statusline#gutentags() abort
    return get(g:, 'loaded_gutentags', 0) ? gutentags#statusline() : ''
endfunction
